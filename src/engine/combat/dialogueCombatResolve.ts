...
export function resolveDialogueChoice(
  ...
  log.push({ kind: 'player_choice', message: pickLocalized(choice.text, getLocale()) });
  ...
  if (res.kind === 'fixed') {
    ...
    log.push({
      kind: 'roll',
      message: combatLog.logTestResult(res.attr, d1, d2, mod, total, res.tn, ok),
      dice: [d1, d2],
      modifier: mod,
      final: total,
    });
    ...
    rollSucceeded = ok;
    nextNodeId = ok ? res.successNodeId : res.failNodeId;
    const curseBit = penalty > 0 ? combatLog.logLuckCursePenalty(penalty) : '';
    log.push({
      kind: 'roll',
      message: combatLog.logLuckResult(d1, d2, mod, curseBit, total, res.tn, ok),
      dice: [d1, d2],
      modifier: mod,
      final: total,
    });
  ...
  for (const eff of effectBlocks) {
    if (eff.playerHpLossPercent != null && eff.playerHpLossPercent > 0) {
      ...
      if (lead) {
        ...
        if (loss > 0) {
          log.push({
            kind: 'leader_damage',
            message: combatLog.logHostilityDrain(loss),
          });
        }
      }
    }
    ...
    if (eff.enemyHpDelta != null && eff.enemyHpDelta !== 0) {
      ...
      log.push({
        ...
        message:
          eff.enemyHpDelta < 0
            ? combatLog.logHostilityCool(-eff.enemyHpDelta)
            : combatLog.logHostilitySharpen(eff.enemyHpDelta),
  ...
  if (tensionHp <= 0) {
    log.push({
      kind: 'info',
      message: combatLog.logHostilityCalm(),
    });
  ...
  log.push({ kind: 'interlocutor_line', message: pickLocalized(nextNode.line, getLocale()) });
